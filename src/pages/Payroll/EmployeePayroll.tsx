...
export default function PayrollList() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const payrollSummary = await PayrollService.fetchPayrollListEmployee(
          {
        ...
        setData(payrollSummary as PayrollSummaryEmployee[] ?? [])
      } catch (err) {
        console.log(err)
          ...
          {currentData.length ?
            currentData.map((payroll, index) => (
              ...
              >
                ...
                <TableCell>{payroll.leaveHour}</TableCell>
